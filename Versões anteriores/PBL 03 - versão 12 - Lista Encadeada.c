...
//====================SESSÃO DECLARAÇÃO DE FUNÇÕES.==================
	...
	int menu_principal();
	int e_igual();
	Problema* cria_lista_e_inserePE();
	...
	do{
		...
		if(verifica_se_vazio(arquivo)){
			...
			for(i = 0; i < qtd_linhas; i++){
				printf("entrou no laço");
				novo_problema = preencher_celula(f1);
				x = e_igual();
				printf("%d", x);
				if(x == 0)
					l = cria_lista_e_inserePE();
				else if(x == 1)
					printf("inseriu p elemento");
				else
...
Problema* preencher_celula(FILE* pegar_no_arq){
	...
	printf("%d", o->valor_de_ordem);
	return o;
}
//==============================================================================================================================================================================
int e_igual(Problema *l, Problema *o){
	
	int contador_de_prob_igual = 0;
	if(l == NULL){
		return 0;
	}
	else{
		Problema *atual = l;
		while(atual != NULL){
			if(o->valor_de_ordem == atual->valor_de_ordem)
				contador_de_prob_igual++;
			
			atual = atual->prox_problema;
		}
		if(contador_de_prob_igual > 0)
			return 1;
		else
			return 2;
	}
...
//==============================================================================================================================================================================
Problema* cria_lista_e_inserePE(Problema* l, Problema* o_novo){
	printf("inseriu p elemento");
...
Problema* insere_ordenado(Problema* l, Problema* o_novo){
	...
	if(l != NULL){
		if(o_novo->valor_de_ordem > l->valor_de_ordem){
