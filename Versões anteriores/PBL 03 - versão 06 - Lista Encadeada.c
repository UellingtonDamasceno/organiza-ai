	...
	do{
		...
		if(verifica_se_vazio(arquivo) == TRUE){
			...
			for(i = 0; i < qtd_linhas; i++){
				novo_problema = preencher_celula(arquivo, f1);
...
//======================================================================================================================================================================================
Problema* preencher_celula(char arquivo[], FILE *pegar_no_arq){
	Problema *o;
...
Problema* insere_ordenado(Problema* l, Problema* o_novo){
	...
	if (l == NULL){//verifica-se, a lista esta vazia. 
		...
		printf("entrou aki 00\n");
	}
	else{
		while(ja_inseriu == FALSE){
			if(prox == NULL){//verifica se esta no final da lista. 
				o_novo->prox_problema = NULL;//se tiver insere o elemento novo.
				ant->prox_problema = o_novo;
				ja_inseriu = TRUE;
				printf("entrou aki 01\n");
			}
			else if(o_novo->ano < prox->ano){//verifica se o novo ano é menor que o proximo;
				ant = prox;//se for ele pula para a proxima verificação;
				prox = prox->prox_problema;
				printf("entrou aki 02\n");
			}
			else if(o_novo->ano > prox->ano){
				o_novo->prox_problema = prox;
				ant->prox_problema = o_novo;
				ja_inseriu = TRUE;
				printf("entrou aki 03\n");
			}
			else{//verifica se o novo ano é igual ao proximo.
				if(o_novo->semestre < prox->semestre){//se for igual ele verifica se o semestre er menor;
					ant = prox;//se for menor ele pula para a proxima posição. 
					prox = prox->prox_problema;// tudo por que o semestre deve ser ordenado por ordem decrescente
					printf("entrou aki 04\n");
				}
				else if(o_novo->semestre > prox->semestre){//se o semestre for maior
					if(o_novo->problema > prox->problema){//verifica-se, o problema er maior. 
						ant = prox;
						prox = prox->prox_problema;
						printf("entrou aki 05\n");
					}
					else if(o_novo->problema < prox->problema){
						o_novo->prox_problema = prox;
						ant->prox_problema = o_novo;
						ja_inseriu = TRUE;
						printf("entrou aki 06\n");
					}
					else{
						free(o_novo);
						ja_inseriu = TRUE;
						printf("entrou aki 07\n");
					}
				}
				else{
					if(o_novo->problema > prox->problema){
						ant = prox;
						prox = prox->prox_problema;
						printf("entrou aki 08\n");
					}
					else if(o_novo->problema < prox->problema){
						o_novo->prox_problema = prox;
						ant->prox_problema = o_novo;
